...
LastEditTime: 2021-08-16 15:26:49
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: /GC-Net/read_data.py
'''
...
class sceneDisp(Dataset):
    ...
    def __init__(self, root_dir,settype,transform=None):
        ...
        finl = open('dataset/paths_left_'+settype+'.pkl','rb')
        finr = open('dataset/paths_right_'+settype+'.pkl', 'rb')
        self.paths_left = pickle.load(finl)
        self.paths_right = pickle.load(finr)
        finl.close()
        finr.close()
        finl = open('dataset/disp_left_'+settype+'.pkl', 'rb')
        finr = open('dataset/disp_right_'+settype+'.pkl', 'rb')
